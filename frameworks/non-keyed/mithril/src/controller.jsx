...
var Controller = {
    ...
    view: function(vnode) {
        let rows = this.data().map((d,i) => {
            ...
            return <tr className={ sel }>
                <td class="col-md-1">{ d.id }</td>
                <td class="col-md-4">
                    <a onclick={ () => this.select(d.id) }>{ d.label }</a>
                </td>
                <td class="col-md-1"><a onclick={ () => this.delete(d.id) } className="remove"><span className="remove glyphicon glyphicon-remove" aria-hidden="true"></span></a></td>
